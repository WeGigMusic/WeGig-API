...
/**
 * Adds artist images to event results using the
 * shared WeGig artist-image service.
 *
 * getArtistImage() checks the WeGig Artist table first,
 * so existing artists do not repeatedly hit Apple/Spotify.
 */
async function addArtistImages(
   ...
   *
   * If several artists are uncached at once,
   * this avoids firing a burst of provider
...
app.get(
  ...
  (
    ...
    res
      ...
      .type("text")
      .send(
        "WeGig API is running. Try /health or /gigs",
      );
  },
);
...
app.post(
  ...
  async (
    ...
    const externalSource =
      typeof gigInput.externalSource ===
        "string" &&
    ...
    const venueLongitude =
      typeof gigInput.venueLongitude ===
        "number" &&
      Number.isFinite(
    ...
    const venuePlaceName =
      typeof gigInput.venuePlaceName ===
        "string" &&
    ...
    try {
      ...
      if (venuePlaceId) {
        ...
        const alreadyByPlaceId =
          existingForArtistDate.find(
            (g) =>
              norm(
                g.artist,
              ) ===
    ...
    try {
      ...
      if (duplicate) {
        return res
          ...
          .json({
            message:
              "This change would create a duplicate gig.",
        ...
        await prisma.gig.update(
          {
            ...
            data: {
              ...
              venuePlaceName:
                next.venuePlaceName?.trim() ||
                null,

              venuePlaceId:
                next.venuePlaceId?.trim() ||
                null,
            },
          },
        );
    ...
    try {
      ...
      if (
        ...
        if (
          ...
          console.error(
            "[events/search] Ticketmaster failed",
            tmResult.reason,
          );
        }
        ...
        if (
          ...
          console.error(
            "[events/search] Skiddle failed",
            skiddleResult.reason,
          );
        }
        ...
        const eventsWithImages =
          await addArtistImages(
            events,
          );

        console.log(
          "[events/search] future results",
          {
            ticketmaster:
              tmEvents.length,
...
app.get(
  ...
  async (
    ...
    try {
      ...
      console.log(
        "[tm/events/search] query",
        req.query,
      );

      const data =
        await searchTmEventsUk(
          {
            keyword:
              kw,
  ...
  async (
    ...
    try {
      ...
      if (
        ...
        console.error(
          "[discover/events] Ticketmaster failed",
          tmResult.reason,
        );
      }
      ...
      if (
        ...
        console.error(
          "[discover/events] Skiddle failed",
          skiddleResult.reason,
        );
      }
    ...
    try {
      ...
      return res
        ...
        .json({
          message:
            e?.message ??
            "Ticketmaster event lookup failed",
        });
    }
  },
);
    ...
    try {
      ...
      return res
        ...
        .json({
          message:
            e?.message ??
            "Ticketmaster venue search failed",
        });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      return res
        ...
        .json({
          message:
            "Failed to fetch from MusicBrainz",
  ...
  async (
    ...
    try {
      ...
      return res
        ...
        .json({
          message:
            "Failed to fetch artist releases from MusicBrainz",
...
/**
 * Shared WeGig artist image endpoint.
 *
 * This should be used by lightweight UI such as:
 * - Gig cards
 * - Discover cards
 * - Stats
 * - Share cards
 *
 * The backend checks the Artist table first.
 * External providers are only called when necessary.
 */
app.get(
  "/artists/image",
  async (
    ...
    try {
      ...
      return res
        ...
        .json({
          message:
            e?.message ??
            "Spotify artist page lookup failed",
        });
    }
  },
);
      ...
      if (
        ...
        return res
          ...
          .json({
            ...
            message:
              "Unable to load setlists right now.",
          });
      }
      ...
      if (
        ...
        return res
          ...
          .json({
            ...
            message:
              "Missing required params: artist and date",
          });
      }

      const result =
        await matchSetlistToGig(
          {
            artist,
            date,
            city,
      ...
      if (
        ...
        return res
          ...
          .json({
            ...
            message:
              "Unable to load setlist right now.",
          });
      }
...
app.listen(
  ...
  () => {
    console.log(
      `WeGig API server running on port ${PORT}`,
    );
  },
);